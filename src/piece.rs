...
impl LegalMoveGen<'_> {
	fn sliding_moves(&mut self) -> Option<()> {
		...
		for direction_index in start_dir_index..end_dir_index {
			...
			for n in 0..self.squares_to_edge.get(&(self.start_square as usize))?[direction_index] {
				let target_square =
	...
	fn knight_moves(&mut self) -> Option<()> {
		let directions = [
			(1, 2),
			(2, 1),
			(-1, 2),
